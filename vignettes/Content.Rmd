...
# Phytochemials in two dimensions
...
all_std <- dplyr::mutate(all_std, 
              Monoisotopic = sapply(Formula,
                                    FUN = function(x){
                                      getMolecule(x)$exactmass
                                    } ) )  %>%  dplyr::rowwise() %>% 
  dplyr::mutate(rt = mean(c(min_rt, max_rt) )/60) %>% 
  left_join(smiles2classyfire)

all_std %>%# filter(!(Name %in% "Naringenin")) %>% 
ggplot2::ggplot( ggplot2::aes(rt, Monoisotopic, color = Superclass_CF)) +
  ...
  ggplot2::theme(legend.position = c(0.82, 0.77), 
                 legend.background = element_rect(fill = "white",
                                                  color = "black")) +
  ggplot2::labs(x = "Retention time (min)", y = "Neutral monoisotopic mass",
                title = "Distribution of the metabolites included in PhenolicsDB",
                subtitle = "Metabolites are colored based on Superclass information from Classifire")
```
